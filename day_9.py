...
def day_9():
    ...
    def find_minima(grid):
        ...
        for x in range(row_max):
            for y in range(column_max):
                adjacent = [grid[x + dx][y + dy] for dx, dy in deltas if
                            0 <= x + dx < row_max and 0 <= y + dy < column_max]
    ...
    def depth_first_search(row, column, grid, visited):
        ...
        if 0 <= row < row_max and 0 <= column < column_max and (row, column) not in visited and grid[row][column] < 9:
            ...
            return 1 + sum(
                depth_first_search(row + d_row, column + d_column, grid, visited) for d_row, d_column in deltas)
    ...
    return part_1, prod(sorted(basin_sizes, reverse=True)[:3])
